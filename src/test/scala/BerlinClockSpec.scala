...
import workshop.codekata.BerlinClock

class BerlinClockSpec extends FlatSpec with Matchers {

  "Yellow light" should "blink on/off every two seconds" in {
  ...
  it should "light a read lamp for every 5 hours" in {
  ...
  it should "light a read lamp for every hour left from top hours" in {
  ...
  it should "light a yellow light for every minute left from top minutes" in {
  ...
  "Berlin Clock" should "result in array with 5 elements" in {
    BerlinClock.convertToBerlinTime("13:17:01").length should be (5)
  }
